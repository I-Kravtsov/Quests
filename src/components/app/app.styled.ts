...
const GlobalStyle = createGlobalStyle`
  ...
  *::after {
  ...
  body {
    ...
    font-style: normal;
    font-weight: 500;
    font-size: ${({ theme }) => appTheme.font.base};
    line-height: 22.5px;
    color: ${({ theme }) => appTheme.color.whisper};
    font-feature-settings: 'pnum' on, 'lnum' on;

    background-color: ${({ theme }) => appTheme.color.nero};
  ...
  input:-webkit-autofill {
    box-shadow: inset 0 0 0 1000px ${({ theme }) => appTheme.color.white};

    -webkit-text-fill-color: ${({ theme }) => appTheme.color.black};
  }
